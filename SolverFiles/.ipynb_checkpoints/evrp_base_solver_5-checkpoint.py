"""
"""
run_instances = ["c101C5", "c103C5", "c206C5", "c208C5", "r104C5", "r105C5", "r202C5", "r203C5", "rc105C5", "rc108C5", "rc204C5", "rc208C5"]
for instance in run_instances:
    ...
    solution_path = Path("./Solution/case_3/small_instances/Cplex5er")
    ...
    # Penalty parameters
    penalty_for_overcharging = 10
    penalty_for_overdischarging = 10
    ...
    mdl.minimize(
        ...
        + penalty_for_overcharging * mdl.sum(
            (battery_overcharged_level[(node_i, vehicle)]) ** 2 for node_i in set_of_all_nodes for vehicle in set_of_vehicles
        ) / battery_capacity_of_vehicle
        + penalty_for_overdischarging * mdl.sum(
            (battery_overdischarged_level[(node_i, vehicle)]) ** 2 for node_i in set_of_all_nodes for vehicle in set_of_vehicles
        ) / battery_capacity_of_vehicle
    )
